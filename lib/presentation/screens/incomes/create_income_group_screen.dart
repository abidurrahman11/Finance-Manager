...
/// Dedicated screen for creating a new income group.
///
/// Mirrors [CreateGroupScreen] for expenses — users first pick the group
/// type (offline or collaborative) via two large selector cards, then fill
/// in title, optional description, and optional date range. No dialogs.
/// The collaborative card is disabled when the user has no remote session
/// and shows a clear "Sign in required" hint.
class CreateIncomeGroupScreen extends ConsumerStatefulWidget {
  const CreateIncomeGroupScreen({super.key});

  @override
  ConsumerState<CreateIncomeGroupScreen> createState() =>
      _CreateIncomeGroupScreenState();
}

class _CreateIncomeGroupScreenState
    extends ConsumerState<CreateIncomeGroupScreen> {
  ...
  Future<void> _submit() async {
    ...
    final bool ok;
    if (_isRemote!) {
      ok = await ref.read(sharedIncomeGroupsProvider.notifier).create(
            title: title,
            description: desc,
      ...
      ok = await ref.read(incomeGroupsProvider.notifier).create(
            title: title,
            description: desc,
            startDate: _startDate,
            endDate: _endDate,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('New Income Group')),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: _TypeCard(
                      ...
                      label: 'Offline',
                      description:
                          'Stored on this device. Great for personal income tracking.',
                  ...
                  Expanded(
                    child: _TypeCard(
                      ...
                      description: hasRemoteSession
                          ? 'Share with others and track income together online.'
                      ...
                      onTap: hasRemoteSession
                          ...
                          : () => context
                              .push('/login?returnTo=/incomes/groups/new'),
                    ),
                  ),
                ],
              ),
              ...
              AppTextField(
                label: 'Group title',
                hint: 'e.g. Freelance income, Household earnings',
              ...
              AppTextField(
                ...
                hint: 'What is this income group for?',
